import numpy as np
import random, argparse, os, re, tqdm, time, uuid
...
def main():
    ...
    if args.problem_source_uid:
        ...
        result_saving_file = os.path.join(args.outdir, step_id+'.jsonl')
    ...
    overall_stats = { 
        ...
        "non_color_invariant": {
            ...
            "non_well_formed": 0, 
            "non_color_invariant": 0
            }, 
        "identity": 0, 
        "non_well_formed_output": 0, 
        "black_output": 0, "timeout": 0, 
        "non_well_formed_input": 0, 
        "duplicate_input": 0, 
        "total": 0
    }
    problems = []
    ...
    if args.jsonl:
        with open(result_saving_file, "w") as f:
            ...
            try:
                with open(result_saving_file, "r") as f:
                    if f.read() =="":
                ...
                sample_records = [{
                    ...
                    "error_message": "",
                    "createAt": datetime.now(timezone.utc),   # ← Current UTC time
...
class GridValidator() :
    # validation checking functions in a single class.
    ...
    @staticmethod
    def check_grids_all_equal(self,grids):
        # check a set of grids are all equal
        assert len(grids) > 0
        return all(np.array_equal(grids[0], grid) for grid in grids)

    @staticmethod
    def check_identity(self, input_grid, output_grid):
        # check if input_grid and output_grid are the same array.
        try:
            assert self.check_grid(input_grid)
            assert self.check_grid(output_grid)
        except:
            breakpoint()
...
def generate_input_grids(problem_source, num_returns=3, timeout=1, function_name="generate_input", retries=20, deduplicate=True):
    ...
    while len(return_input_grids) < num_returns and tries < retries:
        ...
        for input_grid in input_grids:
            if not GridValidator.check_grid(input_grid):
                tries += 1
...
def generate_problem(problem_source, num_input_grids=10, num_deterministic_check=20, num_color_permute_check=20, timeout=1, total_timeout=30):
    """
    ...
    for input_grid in input_grids:
        ...
        if len(output_grids) == 0:
            ...
            continue
        if not GridValidator.check_grids_all_equal(output_grids):
            print("Non-deterministic transformation")
            stats["non_deterministic"] += 1
            stats["total"] += 1
            return None, stats
        if not all(GridValidator.check_grid(output_grid) for output_grid in output_grids):
        ...
        for permuted_output_grid, color_mapping in zip(permuted_output_grids, color_mappings):
            if not GridValidator.check_grid(permuted_output_grid) or not GridValidator.check_grid(input_grid):
                print("Permute check failed due to non-well-formed grids")
                stats["non_color_invariant"]["non_well_formed"] += 1
                stats["total"] += 1
                return None, stats
            if not GridValidator.check_identity(apply_color_mapping(expected_output_grids, color_mapping), permuted_output_grid):
        ...
        if GridValidator.check_identity(input_grid, output_grid):
