...
MODEL="o3-mini"
TARGET="relational-pattern"
TEST="float-line"

mkdir -p "[intergrated]generated_descriptions/$TARGET"
...
for filename in `ls "[intergrated]generated_descriptions/$TARGET/"*jsonl`; do python generate_visualization_html.py   --test $TEST  --outdir $TEST-$MODEL-generated_code/$TARGET --ignore_cache_samples --prompt_model $MODEL --max_tokens 40000 -n 2 -s 4 --nohtml --jsonl $filename; done
