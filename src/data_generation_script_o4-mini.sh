...
for TARGET in "${TARGETS[@]}"; do
  ...
  for filename in `ls $MODEL-generated_descriptions/$TARGET/*jsonl`; do 
    python generate_visualization_html.py --outdir $MODEL-generated_code/$TARGET --ignore_cache_samples --prompt_model $MODEL --max_tokens 40000 -n 2 -s 4 --nohtml --jsonl $filename
  done
