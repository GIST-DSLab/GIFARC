...
# concepts:
# objects, color guide, masking, scaling, rotation

# description:
# In the input you will see a large monochromatic object and a smaller multicolored square object.
# To make the output, first downscale the large object so that it is the same size as the small multicolor square.
# Then, rotate the downscaled object 90 degrees clockwise.
# Finally, use the rotated object as a binary mask to zero out pixels in the square, but instead of making masked pixels black,
# replace them with the color of the large object in the input.
# Return the resulting masked and color-replaced sprite.
...
def main(input_grid):
    ...
    # rotate the scaled pattern 90 degrees clockwise
    rotated_pattern = np.rot90(scaled_pattern, k=-1)

    # get the color of the large object (pattern)
    pattern_color = next(color for color in set(pattern.flatten()) if color != Color.BLACK)

    # if the pixel is in the rotated pattern, keep the color from the square
    # otherwise replace it with the color of the large object in the output grid
    output_grid = np.where(rotated_pattern, square_sprite, pattern_color)
